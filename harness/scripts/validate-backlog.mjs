...
if(current.active_baseline===null){
  ...
  const activeEntries=(reg.entries||[]).filter(e=>(e.contract_status==="ACTIVE" && e.required_for_build_freeze===true) || e.contract_status==="READY_FOR_IMPLEMENTATION");
  ...
  for(const e of activeEntries){
    if(!claimed.has(e.acceptance_id)) errors.push("Missing implementation-ready Acceptance coverage: "+e.acceptance_id);
  }
  if(claimed.size!==activeEntries.length){
    errors.push("Implementation-ready Acceptance coverage mismatch: mapped "+claimed.size+" / required "+activeEntries.length);
  }
}
